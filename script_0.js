...
let allWorkers=[],allTrades=[],allAreas=[],photosByWorker={},allReviews=[],ratingsByWorker={},pendingReviewsByWorker={},adminNotifications={},dashboardStats={};
...
async function loadAllData(){await loadTrades();await loadAreas();await loadWorkers();await loadPhotosForAll();await loadReviewsAdmin();await loadNotifications();await loadDashboardStats(false);buildRatingMaps();fillTradeSelects();fillAreaSelects();renderWorkers(allWorkers);stats()}
...
async function loadDashboardStats(showToast){
  try{
    const data = await fetchJson(["/api/admin/dashboard-stats"]);
    dashboardStats = data || {};
    renderDashboardStats();
    if(showToast) toast("success","تم تحديث الإحصائيات");
  }catch(e){
    dashboardStats = {};
    renderDashboardStats();
    if(showToast) toast("error","تعذر تحميل الإحصائيات");
  }
}

function moneyText(v){return (Number(v)||0).toLocaleString("ar-EG") + " جنيه"}
...
function renderDashboardStats(){
  ...
  setText("dashTotalWorkers", workers.total || 0);
  setText("dashApprovedWorkers", workers.approved || 0);
  setText("dashPendingWorkers", workers.pending || 0);
  setText("dashFeaturedWorkers", workers.featured || 0);
  setText("dashActiveSubs", subs.active || 0);
  setText("dashSoonSubs", subs.soon || 0);
  setText("dashExpiredSubs", subs.expired || 0);
  setText("dashPendingReviews", reviews.pending || 0);
  setText("dashTotalPaid", moneyText(payments.totalAmount || 0));
  setText("dashMonthPaid", moneyText(payments.monthAmount || 0));
  setText("dashPaymentCount", payments.count || 0);
  setText("dashAveragePayment", moneyText(payments.averageAmount || 0));

  renderDashboardList("dashTopTrades", d.topTrades || [], "حرفة");
  renderDashboardList("dashTopAreas", d.topAreas || [], "منطقة");
  renderRecentPayments(payments.recent || [], d.paymentsWarning || "");
}

function renderDashboardList(id,items,label){
  const box=document.getElementById(id);
  if(!box)return;
  if(!items.length){box.innerHTML=`<div class="dashboard-muted">لا توجد بيانات ${label} كافية حاليًا.</div>`;return;}
  box.innerHTML=items.map(x=>`<div class="dashboard-row"><span>${x.name||"غير محدد"}</span><small>${x.count||0}</small></div>`).join("");
}

function renderRecentPayments(items,warning){
  const box=document.getElementById("dashRecentPayments");
  if(!box)return;
  if(warning){box.innerHTML=`<div class="dashboard-muted">${warning}</div>`;return;}
  if(!items.length){box.innerHTML='<div class="dashboard-muted">لا توجد عمليات دفع محفوظة حتى الآن.</div>';return;}
  box.innerHTML=`<table class="dashboard-table"><thead><tr><th>الصنايعي</th><th>المبلغ</th><th>الطريقة</th><th>الحالة</th><th>التاريخ</th></tr></thead><tbody>${items.map(x=>`<tr><td>${x.worker_name||"غير محدد"}</td><td>${moneyText(x.amount)}</td><td>${paymentMethodLabel(x.payment_method)}</td><td>${paymentStatusLabel(x.payment_status)}</td><td>${formatDate(x.created_at)}</td></tr>`).join("")}</tbody></table>`;
}
